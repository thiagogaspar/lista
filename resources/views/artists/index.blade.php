...
<div class="flex items-center gap-3 mb-4">
    <h1 class="text-2xl font-bold text-base-content">Artists</h1>
...
<div class="lg:flex lg:gap-8">
    <div class="flex-1 min-w-0">
        <form method="GET" class="mb-3">
            <input name="search" value="{{ request('search') }}" placeholder="Search artists..."
        ...
        <div class="flex items-center justify-between mb-3 text-xs text-base-content/50">
            ...
            <div class="flex gap-3">
                <a href="?sort=name&dir={{ $sort === 'name' && $dir === 'asc' ? 'desc' : 'asc' }}&{{ $qs }}" class="hover:text-secondary {{ $sort === 'name' ? 'font-semibold text-secondary' : '' }}">Name {{ $sort === 'name' ? ($dir === 'asc' ? '↑' : '↓') : '' }}</a>
                <a href="?sort=origin&dir={{ $sort === 'origin' && $dir === 'asc' ? 'desc' : 'asc' }}&{{ $qs }}" class="hover:text-secondary {{ $sort === 'origin' ? 'font-semibold text-secondary' : '' }}">Origin {{ $sort === 'origin' ? ($dir === 'asc' ? '↑' : '↓') : '' }}</a>
            </div>
        </div>
        ...
        <div class="mt-4 flex items-center justify-between text-sm text-base-content/50">
            ...
            <div class="join">{{ $artists->appends(request()->query())->links('pagination::tailwind') }}</div>
        </div>
    </div>
    ...
    <aside class="lg:w-64 mt-6 lg:mt-0 shrink-0 lg:sticky lg:top-20 self-start">
        <div class="card card-compact bg-base-100 border border-base-300">
            <div class="card-body p-4">
                ...
                <p class="text-xs text-base-content/50">Total artists: {{ $artists->total() }}</p>
            </div>
        </div>
